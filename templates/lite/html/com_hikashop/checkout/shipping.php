...
?>
<section id="hikashop-shipping">
<div class="panel">
<fieldset>
  <div class="panel-heading">
    <h3 class="panel-title"><?php echo JText::_('HIKASHOP_SHIPPING_METHOD');?></h3>
  </div>
...
<?php
			foreach($group->products as $product) {
				...
				if(!empty($image->success)) {
					echo '<li class="list-group-item"><img src="' . $image->url . '" alt="' . $name . '" title="' . $name . '"/></li>';
				} else {
					echo '<li class="list-group-item"><span>'.$product->product_name.'</span></li>';
				}
			}
?>
		</ul>
...
<?php if(!HIKASHOP_RESPONSIVE) { ?>
            ...
            <label for="hikashop_shipping_<?php echo $rate->shipping_type.'_'.$rate->shipping_id . $shipping_key;?>" style="cursor:pointer;">
